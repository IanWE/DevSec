...
#define fatal(fmt,args...) do { log_err(fmt,##args); goto err_exit; } while(0)
    ...
    while(!found && fgets(buff, sizeof(buff), maps))
        if(strstr(buff,"-p") && strstr(buff,libpath)) found = 1;
    ...
    shoff = (void *)((size_t) elf + elf->e_shoff);

    for(k = 0; k < elf->e_shnum; k++, shoff = (void*)((size_t)shoff + elf->e_shentsize))  {
        ...
        switch(sh->sh_type) {
            case SHT_DYNSYM:
                ...
                memcpy(ctx->dynsym, (void *)((size_t)elf + sh->sh_offset), sh->sh_size);
            ...
            case SHT_STRTAB:
                ...
                memcpy(ctx->dynstr, (void *)((size_t) elf + sh->sh_offset), sh->sh_size);
                break;
...
void *fake_dlsym(void *handle, const char *name, size_t& offset)
{
    int k;
    struct ctx *ctx = (struct ctx *) handle;
    ...
    for(k = 0; k < ctx->nsyms; k++, sym++)
        if(strcmp(strings + sym->st_name, name) == 0) {
            ...
            void *ret = (void*)((size_t)ctx->load_addr + sym->st_value - ctx->bias);
            offset = sym->st_value;
            log_info("%s found at %p and handle is %p, st_value %p,bias %p", strings + sym->st_name, ret, ctx->load_addr,sym->st_value,ctx->bias);
            return ret;
        }
    return 0;
}
