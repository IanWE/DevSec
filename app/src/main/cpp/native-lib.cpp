...
int (*hit)(int, int*, pthread_mutex_t*, size_t *,size_t, int*,int*, size_t*, int, long*, int*, int*, int*);
void (*acs)(void*);
...
Java_com_SMU_DevSec_SideChannelJob_scan(
    ...
    hit = (int (*)(int, int*,pthread_mutex_t* , size_t *, size_t, int*, int*, size_t*, int, long*, int*, int*, int*)) dlsym(handle, "hit");
    if (!hit)  {
    ...
    */
    ...
    //addr[5] += 0;
    hit(cpu,&threshold,&g_lock,addr,sum_length,flags,thd,&continueRun,compiler_position,times,logs,thresholds,&log_length);// start scaning
